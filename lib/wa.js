...
class WhatsApp {
  constructor(options) {
    ...
    this.events = {}
    this.cache_lid = []
    this.cache_lid_loop = null
    this.cache_lid_isenable = configDefault.balieyscontent?.cacheingLID || false
    this.fn_key = []
  ...
  async WhatsAppClientStartUp() {
    ...
    const DefaultConnectionUpdate = async (connect_wa) => {
      ...
      if(connection === "close") {
        ...
        // Delete Loop Update Cachc LID
        if(this.cache_lid_loop) {
          clearTimeout(this.cache_lid_loop)
        }
        // Other Handle
        if(tryConnect) {
          setTimeout(() => {
      ...
      if(connection === "open") {
        const updateCacheLID = async () => {
          // --- UPDATE LID USER [TO HARD IMPLEMENTATION] ---
          // Not recommended because it will affect resources/RAM usage. Cache stored in variables that are not cleared for a long time will cause data to become full.
          const cache_loop_age = 1000 * 60 * 60 * 30 // Update Per 30 Minutes
          const groupfield = await socket.groupFetchAllParticipating()
          this.cache_lid = MessageApplyParticipants(groupfield)
          this.cache_lid_loop = setTimeout(updateCacheLID, cache_loop_age)
        }
        if(this.cache_lid_isenable) {
          updateCacheLID()
        }
          ...
          const EventsCalling = (data) => {
            ...
            if(eventName === "message") {
              const contentMsg = ModiferMessages(
                data.messages[0],
                socket,
                this.whatsappconfig.perfix,
                this.cache_lid // LID Cacheing
              )
